...
void Reception::printStatus() {
    ...
    for (size_t i = 0; i < _kitchens.size(); i++) {
            ...
            while (std::chrono::steady_clock::now() < deadline) {
                ...
                if (_kitchens[i].channel->tryRead(m)) {
                    if (m.type == MsgType::STATUS_RES) {
                        ...
                        LOG_INFO(std::string("[READY] ") +
                                 pizzaTypeName(m.pizzaType) + " " +
        ...
        if (got) {
            ...
            summary << "Kitchen #" << i + 1 << ": " << res.cooksTotal
                    << " cooks, " << res.cooksBusy << " busy, "
                    << res.pizzasQueued << " pizzas in queue";
